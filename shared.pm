...
sub getserverconfig {
    my $variable = shift;
    my $sql = "SELECT * from `server_config` WHERE `variable` like ? limit 1;";
	my $configdata=$dbh->selectrow_hashref($sql, undef, ($variable));
    return $configdata->{value};
}
sub setserverconfig {
    my $variable = shift;
    my $value = shift;
    my $sql = "UPDATE `server_config` SET value = ? WHERE `variable` like ? limit 1;";
	my $configdata=$dbh->do($sql, undef, ($value, $variable));
    return $configdata;
}
sub init {
	initquiet();
